...
Poisson distributie lijkt een goede fit voor alfa-diversiteit Annelida (zie notes meeting 18/04/2024).
...
### Staalname maand als verklarende variabele
...
Wanneer we dit model fitten met interactie tussen maand en landgebruik, zijn er waarschuwingen die aangeven dat de interactie tussen maand en landgebruik niet mogelijk is.
Dit is te verwachten omdat landgebruik en maand niet volledig gekruist zijn.
We hebben deze interactie daarom verwijderd uit het model.
...
We kunnen nu predicties maken van soortenrijkdom waarbij we controleren voor het totaal aantal reads in een staal en de maand waarin het staal genomen is:
...
```{r m-annelida-shannon}
samples_totcount_10k <- tidy_physeq$samples %>%
  filter(total_count > 1e+4, Shannon > 0)
...
```{r m-annelida-shannon-summary-gamma}
summary(m_annelida_richness)
```

```{r m-annelida-shannon-anova-gamma}
car::Anova(m_annelida_richness)
```
...
```{r m-annelida-shannon-predictions-gamma}
marginaleffects::plot_predictions(
  m_annelida_richness,
  condition = c("Landgebruik_MBAG", "Diepte"),
  vcov = vcov(m_annelida_richness),
...
```{r m-annelida-shannon-incl-staalname-maand}
samples_totcount_10k <- tidy_physeq$samples %>%
  filter(total_count > 1e+4, Shannon > 0)
...
```{r shannon-incl-staalname-maand}
performance::check_model(m_annelida_richness)
```


```{r m-annelida-shannon-summary-gamma-incl-staalname-maand}
summary(m_annelida_richness)
```

```{r m-annelida-shannon-anova-gamma-incl-staalname-maand}
car::Anova(m_annelida_richness)
```


We kunnen nu predicties maken van soortenrijkdom waarbij we controleren voor het totaal aantal reads in een staal en de maand waarin de staalname plaatsvond:

```{r m-annelida-shannon-predictions-gamma-incl-staalname-maand}
marginaleffects::plot_predictions(
  m_annelida_richness,
  condition = c("Landgebruik_MBAG", "Diepte"),
  vcov = vcov(m_annelida_richness),
...
op basis van enkel de data van de gekende soorten (tidy_physeq_known_species)

Dit is dus op basis van enkel de OTUs waarvan we weten tot welke soort ze behoren, de 'unknown' OTUs zijn voor deze analyse verwijderd uit de dataset.

Het geobserveerde aantal taxa neemt toe met het aantal reads in een staal:

```{r}
tidy_physeq_known_species$samples %>%
  filter(total_count > 0) %>%
  ggplot(
    aes(x = total_count, y = observed)) +
  geom_point() +
...
```{r m-annelida-richness-known-species}
samples_totcount_filtered <- tidy_physeq_known_species$samples %>%
  filter(total_count > 1e+4)
...
```{r m-annelida-richness-summary-known-species}
summary(m_annelida_richness)
```

```{r m-annelida-richness-anova-known-species}
car::Anova(m_annelida_richness)
```
...
```{r m-annelida-richness-predictions-known-species}
marginaleffects::plot_predictions(
  m_annelida_richness,
  condition = c("Landgebruik_MBAG", "Diepte"),
  vcov = vcov(m_annelida_richness),
...
```{r m-annelida-richness-knownspecies2}
samples_totcount_10k <- tidy_physeq_known_species$samples %>%
  filter(total_count > 1e+4)
...
```{r m-annelida-richness-knownspecies-summary-2}
summary(m_annelida_richness)
```

```{r m-annelida-richness-knownspecies-anova-2}
car::Anova(m_annelida_richness)
```

We kunnen nu predicties maken van soortenrijkdom waarbij we controleren voor het totaal aantal reads in een staal en de maand waarin het staal genomen is:

```{r m-annelida-richness-predictions-known-species-2}
marginaleffects::plot_predictions(
  m_annelida_richness,
  condition = c("Landgebruik_MBAG", "Diepte"),
  vcov = vcov(m_annelida_richness),
