...
library(sccomp)
library(cmdstanr)
set_cmdstan_path("/staging/leuven/stg_00184/genetic_diversity/software/.cmdstan/cmdstan-2.37.0")

# mbag_folder <- "/staging/leuven/stg_00184/genetic_diversity/projects/MBAG_InseKP/sccomp_analysis" # nolint

system("rclone version") # make sure rclone module is loaded in the pre-run scriplet! (module load rclone)
system("rclone mount PRJ_MBAG: ~/rclone_mnt/PRJ_MBAG -vv --daemon") # loads some minutes
system("ls ~/rclone_mnt/PRJ_MBAG")

mbag_folder="~/rclone_mnt/PRJ_MBAG/"
...
Check in output directory for "correct/latest" model

```{r eval=FALSE}
myff = list.files(
    sccomp_dir, full.names = T, pattern = "glm_multi_beta_binomial")[1]

test_as_cmdstan_fit <- cmdstanr::as_cmdstan_fit( myff)
```

This does not work for Nick on HPC (test is an environment?)
```{r eval=FALSE}
typeof(test)
test
```
