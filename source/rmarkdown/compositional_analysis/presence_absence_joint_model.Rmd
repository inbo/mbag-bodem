...
ordiplot(
  constr_ord, symbols = TRUE, s.colors = ann_wide$diepte,
  ind.spp = NULL,
  biplot = TRUE
)
...
sccomp_effects <- m1_contrast |>
  ...
  select(genus, diepte, contrast, effect_sign) %>%
...
gllvm_effects <- constr_ord_contrasts %>%
  ...
  ungroup() %>%
  select(genus = species, diepte, contrast, effect_sign) %>%
  mutate(method = "gllvm")
