...
-   another model where the read counts are modeled directly
...
But even if we can account for it, there are still important open questions regarding the interpretation of the patterns and subsetting (modeling a subset of taxa).
...
For compositional data, the principle of subcompositional coherence (ref Aitchison) states that - if we have compositional data, say with components A, B, C and D, the statistical relationship we observe between component A and B should be _similar_ whether we analyze just A and B, or analyze A, B, and C.
...
For `gllvm`, we used the 95% confidence intervals obtained via the `emmeans` package to declare a contrast signficant if the interval did not contain zero. These confidence intervals were only adjusted for multiple testing within a taxon (dunnettx method for 4 estimates), no FDR adjustments were made across taxa.
...
(ref:comp-not-filtered) Comparison of effect sizes of the difference contrasts for analysis based on presence absence data (modeled via `gllvm`) or count data (modeled via `sccomp`)
...
run_gllvm_analysis <- function(counts, condition) {
  x_df <- data.frame(condition = condition) %>%
    mutate(
      sample = paste0("s", 1:n())
    )
...
data <- generate_counts_condition(
  ...
  n_taxa = n_taxa,
  condition = condition,
  fold_changes = exp(rnorm(n_taxa, mean = 0, sd = 0.3)) # Some taxa up, some down
)
...
-   `sccomp`:
    -   counts are modeled as sum-constrained beta-binomial distributions with logit link
...
-   `gllvm`:
    -   counts are modeled as negative-binomial distributions with log link
    -   correlation among species is modeled via two latent factors
