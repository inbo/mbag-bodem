...
```{r calc-sccomp-predictions-all-species, eval=FALSE}
# maak data.frame voor nieuwe predicties
my_new_data <- sccomp_data_collembola %>%
  distinct(Landgebruik_MBAG, Diepte)
my_new_data$sample_id <- paste0("newsample", seq_len(nrow(my_new_data)))
...
```{r plot-sccomp-predictions-all-species, eval=FALSE}
...
```{r sccomp-test-all-species, eval=FALSE}
sctest <- sccomp::sccomp_test(s_collembola_207_species)
plots <- plot(sctest)
```

```{r sccomp-taxon-info-all-species, eval=FALSE}
sccomp_data_collembola %>%
  group_by(taxon_id, species, occurrence) %>%
  summarise(sum_reads = sum(count)) %>%
  kable()
...
```{r sccomp-boxplot-landgebruik-all-species, eval=FALSE}
plots$boxplot[[1]]
```

```{r sccomp-boxplot-diepte-all-species, eval=FALSE}
plots$boxplot[[2]]
```
...
```{r sccomp-ci1d-all-species, fig.height = 200/25.4, eval=FALSE}
plots$credible_intervals_1D
```
...
```{r sccomp-mean-variance-all-species, eval=FALSE}
plots$credible_intervals_2D
```
