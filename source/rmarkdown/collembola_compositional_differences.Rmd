...
if (!file.exists(here("source", "brms_models", "s_collembola_genus.rds"))) {
  s_collembola_genus <- sccomp_estimate(
  ...
  saveRDS(s_collembola_genus,
          here("source", "brms_models", "s_collembola_genus.rds"))
}

s_collembola_genus <- readRDS(here("source", "brms_models", "s_collembola_genus.rds"))
```

### Model evaluatie

```{r sccomp-test}
sctest <- sccomp::sccomp_test(s_collembola_genus)
plots <- plot(sctest)
```
...
-   in `t3` (meest abundante genus Megalothorax) bij natuurgrasland is er sprake van bimodaliteit: de boxplot zit in de zone met proporties tussen 0 en 0.5, terwijl er een groep van punten is die clustert tussen 0.75 en 1. Na te gaan of deze outliers kunnen verklaard worden met een ontbrekende covariaat (indien niet, kunnen we ze eventueel uit de analyse verwijderen).
...
We zien een duidelijke toename van _Megalothorax_, en een afname van _Isotomidae_ (inclusief Isotomurus) met afnemende landgebruiksintensiteit? Ook zien we een duidelijke toename van verschillende Entomobryidae soorten van akker en tijdelijke grasland naar blijvend grasland. Sommige genera lijken meer voor te komen in 0-10 cm, anderen dan weer in 10-30 cm?
...
We zien een duidelijke toename van _Megalothorax_, en een duidelijke afname van _Folsomia_ en _Isotomurus_ met afnemende landgebruiksintensiteit.   
...
## `sccomp`: differentiële samenstelling en variabiliteitsanalyse op genusniveau met enkel gekende genera



```{r sccomp-inputs-known-genus}
max_taxa <- 10
used_rank <- "genus"
sccomp_data_collembola <- tidy_physeq_known_genera %>%
  aggregate_taxa(rank = used_rank) %>%
  tidytacos::add_prevalence() %>%
  tidytacos::mutate_taxa(
    keep = min_rank(desc(occurrence)) < max_taxa
...
Voor deze analyse werken we enkel met de OTUs die geïdentificeerd zijn tot op genusniveau, aggregeren we de taxa tot `r used_rank` niveau en beperken we de analyse tot de `r max_taxa` taxa met hoogste prevalentie (binnen deze subset van de data).
Dit is goed voor `r round(sum(sccomp_data_collembola$count) / sum(tidy_physeq_known_genera$counts$count) * 100)` % van alle reads in deze subset.

```{r sccomp-Collembola-known-genus}
fs::dir_create(here("source", "brms_models"))
if (!file.exists(here("source", "brms_models", "s_collembola_known_genera.rds"))) {
  s_collembola_known_genera <- sccomp_estimate(
    .data = sccomp_data_collembola,
    formula_composition = ~
      Landgebruik_MBAG
    + Diepte
  ...
  saveRDS(s_collembola_known_genera,
          here("source", "brms_models", "s_collembola_known_genera.rds"))
}

s_collembola_known_genera <- readRDS(here("source", "brms_models", "s_collembola_known_genera.rds"))
```

### Model evaluatie

```{r sccomp-test-known-genus}
sctest <- sccomp::sccomp_test(s_collembola_known_genera)
plots <- plot(sctest)
```

```{r sccomp-taxon-info-known-genus}
sccomp_data_collembola %>%
  group_by(taxon_id, genus, occurrence) %>%
  summarise(sum_reads = sum(count)) %>%
  kable()
...
-   er zijn mogelijk verschillen in variantie tussen de groepen: we kunnen dus eventueel de variantie ook modelleren in functie van deze factoren
-   in `t1` :Megalothorax (en eventueel ``) bij natuurgrasland (en deze keer ook bij tijdelijk grasland?) is er sprake van bimodaliteit: de boxplot zit in de zone met proporties tussen 0 en 0.5, terwijl er een groep van punten is die clustert tussen 0.75 en 1. Na te gaan of deze outliers kunnen verklaard worden met een ontbrekende covariaat (indien niet, kunnen we ze eventueel uit de analyse verwijderen).

```{r sccomp-boxplot-landgebruik-known-genus}
plots$boxplot[[1]]
```

```{r sccomp-boxplot-diepte-known-genus}
plots$boxplot[[2]]
```
...
```{r sccomp-ci1d-known-genus, fig.height = 200/25.4}
plots$credible_intervals_1D
```
...
```{r sccomp-mean-variance-known-genus}
plots$credible_intervals_2D
```
...
```{r calc-sccomp-predictions-known-genus}
# maak data.frame voor nieuwe predicties
my_new_data <- sccomp_data_collembola %>%
  distinct(Landgebruik_MBAG, Diepte)
my_new_data$sample_id <- paste0("newsample", seq_len(nrow(my_new_data)))
...
sortering <- s_collembola_known_genera %>%
  filter(parameter == "Landgebruik_MBAGNatuurgrasland") %>%
  select(taxon_id, c_lower) %>%
  left_join(sccomp_data_collembola %>%
              distinct(taxon_id, genus),
...
De volgende figuur toont de modelvoorspellingen.
We zien een duidelijke toename van _Megalothorax_, en een duidelijke afname van _Folsomia_ en _Isotomurus_ met afnemende landgebruiksintensiteit. Ook zien we een duidelijke toename van _Dicranocentrus_ van akkers en tijdelijke graslanden naar blijvend grasland. En een significant hogere kans op _Protaphorura_ in blijvend grasland en natuurgrasland? Sommige genera lijken meer voor te komen in 0-10 cm, anderen dan weer in 10-30 cm?

```{r plot-sccomp-predictions-known-genus}
s_collembola_predictions %>%
  ggplot() +
  geom_pointrange(
    aes(
...
We zien een duidelijke toename van _Megalothorax_, _Protaphorura_, en een duidelijke afname van _Folsomia_ en _Isotomurus_ met afnemende landgebruiksintensiteit. En de duidelijke toename van _Dicranocentrus_ van akkers en tijdelijke graslanden naar blijvend grasland. 
...
fs::dir_create(here("source", "brms_models"))
if (!file.exists(here("source", "brms_models", "s_collembola_species.rds"))) {
  s_collembola_species <- sccomp_estimate(
    .data = sccomp_data_collembola,
    formula_composition = ~
      Landgebruik_MBAG
    + Diepte
  ...
  saveRDS(s_collembola_species,
          here("source", "brms_models", "s_collembola_species.rds"))
}

s_collembola_species <- readRDS(here("source", "brms_models", "s_collembola_species.rds"))
```

### Model evaluatie

```{r sccomp-test-species}
sctest <- sccomp::sccomp_test(s_collembola_species)
plots <- plot(sctest)
```
...
-   er zijn mogelijk verschillen in variantie tussen de groepen: we kunnen dus eventueel de variantie ook modelleren in functie van deze factoren
-   Sprake van bimodaliteit? In 't292' ( _Megalothorax_ _incertus_) akker? Maar wel verdwenen voor _Megalothorax_ ( _incertus_) dus in natuurgraslanden?: de boxplot zit in de zone met proporties tussen 0 en 0.25, terwijl er een groep van punten is die clustert tussen 0.25 en 1, en vooral tusen 0.5 en 1. Na te gaan of deze outliers kunnen verklaard worden met een ontbrekende covariaat (indien niet, kunnen we ze eventueel uit de analyse verwijderen)?
-   Idem voor _Folsomia_ _candida_ 't298' in tijdelijk grasland? de boxplot zit in de zone met proporties tussen x en x, terwijl er een groep van punten is die clustert tussen y en y
...
```{r sccomp-ci1d-species, fig.height = 200/25.4}
plots$credible_intervals_1D
```
...
sortering <- s_collembola_species %>%
  filter(parameter == "Landgebruik_MBAGNatuurgrasland") %>%
  select(taxon_id, c_lower) %>%
  left_join(sccomp_data_collembola %>%
...
## `sccomp`: differentiële samenstelling en variabiliteitsanalyse op soortniveau met enkel gekende soorten



```{r sccomp-inputs-known-species}
max_taxa <- 10
used_rank <- "species"
sccomp_data_collembola <- tidy_physeq_known_species %>%
  aggregate_taxa(rank = used_rank) %>%
  tidytacos::add_prevalence() %>%
  tidytacos::mutate_taxa(
    keep = min_rank(desc(occurrence)) < max_taxa
...
Voor deze analyse werken we enkel met de OTUs die geïdentificeerd zijn tot op soortniveau, aggregeren we de taxa tot `r used_rank` niveau en beperken we de analyse tot de `r max_taxa` taxa met hoogste prevalentie (binnen deze subset van de data).
Dit is goed voor `r round(sum(sccomp_data_collembola$count) / sum(tidy_physeq_known_species$counts$count) * 100)` % van alle reads in deze subset.

```{r sccomp-Collembola-known-species}
fs::dir_create(here("source", "brms_models"))
if (!file.exists(here("source", "brms_models", "s_collembola_known_species.rds"))) {
  s_collembola_known_species <- sccomp_estimate(
    .data = sccomp_data_collembola,
    formula_composition = ~
      Landgebruik_MBAG
    + Diepte
  ...
  saveRDS(s_collembola_known_species,
          here("source", "brms_models", "s_collembola_known_species.rds"))
}

s_collembola_known_species <- readRDS(here("source", "brms_models", "s_collembola_known_species.rds"))
```

### Model evaluatie

```{r sccomp-test-known-species}
sctest <- sccomp::sccomp_test(s_collembola_known_species)
plots <- plot(sctest)
```

```{r sccomp-taxon-info-known-species}
sccomp_data_collembola %>%
  group_by(taxon_id, species, occurrence) %>%
  summarise(sum_reads = sum(count)) %>%
  kable()
...
-   er zijn mogelijk verschillen in variantie tussen de groepen: we kunnen dus eventueel de variantie ook modelleren in functie van deze factoren
-   Op het eerste zicht geen sprake van bimodaliteit??? : de boxplot zit in de zone met proporties tussen x en x.x, terwijl er een groep van punten is die clustert tussen y.y en y. Na te gaan of deze outliers kunnen verklaard worden met een ontbrekende covariaat (indien niet, kunnen we ze eventueel uit de analyse verwijderen).

```{r sccomp-boxplot-landgebruik-known-species}
plots$boxplot[[1]]
```

```{r sccomp-boxplot-diepte-known-species}
plots$boxplot[[2]]
```
...
```{r sccomp-ci1d-known-species, fig.height = 200/25.4}
plots$credible_intervals_1D
```
...
```{r sccomp-mean-variance-known-species}
plots$credible_intervals_2D
```
...
```{r calc-sccomp-predictions-known-species}
# maak data.frame voor nieuwe predicties
my_new_data <- sccomp_data_collembola %>%
  distinct(Landgebruik_MBAG, Diepte)
my_new_data$sample_id <- paste0("newsample", seq_len(nrow(my_new_data)))
...
sortering <- s_collembola_known_species %>%
  filter(parameter == "Landgebruik_MBAGNatuurgrasland") %>%
  select(taxon_id, c_lower) %>%
  left_join(sccomp_data_collembola %>%
              distinct(taxon_id, species),
...
De volgende figuur toont de modelvoorspellingen.
Duidelijk geen toename van _Megalothorax_ _incertus_ met afnemende landgebruiksintensiteit. Integendeel, duidelijk lager in natuurgrasland. Wel een duidelijke toename van _Megalothorax_ _minimus_, _Isotoma_ _viridis_, _Protaphorura_ _armata_, en _Megalothorax_ _willemi_ (deze laatste enkel in de 10-30) met afnemende landgebruiksintensiteit. Duidelijke afname van _Folsomia_ _candida_ en _Isotomurus_ _maculatus_ (deze laatste enkel op basis van de 0-10 cm) met afnemende landgebruiksintensiteit.
_Megalothorax_ _incertus_, _Megalothorax_ _willemi_, en _Folsomia_ _candida_ lijken meer voor te komen in 10-30 cm dan 0-10 cm, terwijl _Willowsia_ _nigromaculata_, _Isotomurus_ _maculatus_, en _Sminthurus_ _viridis_ meer voor lijken te komen in 0-10 cm, dan 10-30 cm.

```{r plot-sccomp-predictions-known-species}
s_collembola_predictions %>%
  ggplot() +
  geom_pointrange(
    aes(
...
## `sccomp`: differentiële samenstelling en variabiliteitsanalyse op soortniveau met alle OTUs met een minimum occurence van 5

```{r sccomp-inputs-all-species}
max_taxa <- 207
used_rank <- "species"
sccomp_data_collembola <- tidy_physeq %>%
  aggregate_taxa(rank = used_rank) %>%
...
Hier dus eens getest met alle OTUs met een occurrence van minstens 5 (in dit geval de 207 meest prevalente OTUs)

```{r sccomp-collembola-all-species, eval=FALSE}
fs::dir_create(here("source", "brms_models"))
if (!file.exists(here("source", "brms_models", "s_collembola_207_species.rds"))) {
  s_collembola_207_species <- sccomp_estimate(
    .data = sccomp_data_collembola,
    formula_composition = ~
      Landgebruik_MBAG
    + Diepte
  ...
  saveRDS(s_collembola_207_species,
          here("source", "brms_models", "s_collembola_207_species.rds"))
}


```

```{r sccomp-collembola-all-species-load, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
s_collembola_207_species <- readRDS(here("source", "brms_models", "s_collembola_207_species.rds"))
```
...
```{r calc-sccomp-predictions-all-species, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
# maak data.frame voor nieuwe predicties
my_new_data <- sccomp_data_collembola %>%
  distinct(Landgebruik_MBAG, Diepte)
my_new_data$sample_id <- paste0("newsample", seq_len(nrow(my_new_data)))
...
sortering <- s_collembola_207_species %>%
  filter(parameter == "Landgebruik_MBAGNatuurgrasland") %>%
  select(taxon_id, c_lower) %>%
  left_join(sccomp_data_collembola %>%
              distinct(taxon_id, species),
...
De volgende figuur toont de modelvoorspellingen.
Duidelijk geen toename van _xx_ _xx_ met afnemende landgebruiksintensiteit. Integendeel, duidelijk lager in natuurgrasland. De voorheen geobserveerde toename van _xx_ met afnemende landgebruiksintensiteit moet dus aan andere _xx_ soorten te wijten zijn? Duidelijke afname van _xx_ _xx_ en _xx_ _xx_ (deze laatste enkel op basis van de 0-10 cm) met afnemende landgebruiksintensiteit
_xx_ _xx_ komt duidelijk meer voor in 10-30 cm dan 0-10 cm.

```{r plot-sccomp-predictions-all-species, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
# Filter out species with "_otu" in their names
filtered_species_data <- s_collembola_predictions %>%
  filter(!grepl("_otu", species))

# Find unique species from the filtered data and split them into chunks of 9
unique_species <- unique(filtered_species_data$species)
species_chunks <- split(unique_species, ceiling(seq_along(unique_species)/9))

# Loop through each chunk and create a plot
plots <- list() # To store the plots
for (i in seq_along(species_chunks)) {
  # Subset the data for the current chunk of species
  subset_data <- filtered_species_data %>% 
...
# Now you can print or save each plot separately
# For example, to view the first set of plots, you can do:
# print(plots[[1]])
# And so on for each plot in 'plots'
```

```{r print-all-plots, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
plots
```
...
```{r sccomp-test-all-species, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
sctest <- sccomp::sccomp_test(s_collembola_207_species)
plots <- plot(sctest)
```

```{r sccomp-taxon-info-all-species, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
sccomp_data_collembola %>%
  group_by(taxon_id, species, occurrence) %>%
  summarise(sum_reads = sum(count)) %>%
  kable()
...
-   er zijn mogelijk verschillen in variantie tussen de groepen: we kunnen dus eventueel de variantie ook modelleren in functie van deze factoren
-   Sprake van bimodaliteit? In 't292' ( _xx_ _xx_) akker? Maar wel verdwenen voor _xx_ ( _xx_) dus in natuurgraslanden?: de boxplot zit in de zone met proporties tussen 0 en 0.25, terwijl er een groep van punten is die clustert tussen 0.25 en 1, en vooral tusen 0.5 en 1. Na te gaan of deze outliers kunnen verklaard worden met een ontbrekende covariaat (indien niet, kunnen we ze eventueel uit de analyse verwijderen)?
-   Idem voor _xx_ _xx_ 't298' in tijdelijk grasland? de boxplot zit in de zone met proporties tussen x en x, terwijl er een groep van punten is die clustert tussen y en y

```{r sccomp-boxplot-landgebruik-all-species, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
plots$boxplot[[1]]
```

```{r sccomp-boxplot-diepte-all-species, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
plots$boxplot[[2]]
```
...
```{r sccomp-ci1d-all-species, fig.height = 200/25.4, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
plots$credible_intervals_1D
```
...
```{r sccomp-mean-variance-all-species, eval=file.exists("here("source", "brms_models", "s_collembola_207_species.rds")")}
plots$credible_intervals_2D
```
