...
### include SWCvol in model


```{r m-richness-including-SWCvol-{{group}}-{{primerset}}-{{unit}}}
form_richness <- formula(
  observed ~
    log(total_count)
  + Landgebruik_MBAG
...
```{r m-richness-including-SWCvol-checks-{{group}}-{{primerset}}-{{unit}}}
performance::check_model(m_richness)
```


```{r m-richness-including-SWCvol-summary-{{group}}-{{primerset}}-{{unit}}}
summary(m_richness) |> print(digits = 2)
```

```{r m-richness-including-SWCvol-anova-{{group}}-{{primerset}}-{{unit}}}
car::Anova(m_richness) |> print(digits = 2)
```

```{r m-richness-including-SWCvol-preds-total-count-{{group}}-{{primerset}}-{{unit}}}
marginaleffects::plot_predictions(
  m_richness,
  condition = c("total_count"),
  re.form = NA,
...
```{r m-richness-including-SWCvol-preds-landgebruikxdiepte-{{group}}-{{primerset}}-{{unit}}}
marginaleffects::plot_predictions(
  m_richness,
  condition = c("landgebruik", "diepte"),
  re.form = NA,
