...
contrasts_landuse_by_depth_response <- contrast(emm_landuse_by_depth, method = "pairwise", infer = c(TRUE, TRUE)) %>%
  as.data.frame()
...
contrasts_landuse_by_depth_table <- contrasts_landuse_by_depth_response %>%
  as_tibble() %>%
  group_by(diepte) %>%
  ungroup() %>%
  select(
...
write_csv(
  contrasts_landuse_by_depth_table,
  file = file.path(output_dir, glue("contrasts_landuse_by_depth_table_response_{group}_{primerset}_{unit}.csv"))
)
