...
inputs <- combined %>%
  ...
  filter(sampling_month %in% c(11, 12, 1, 2, 3, 4))
...
inputs <- combined %>%
  ...
  filter(sampling_month %in% c(12, 1, 2, 3, 4))
...
if (exists("summary_all") && nrow(summary_all) > 0) {
  ...
  readr::write_csv(summary_all, file.path(output_dir, "richness_vs_c_density_all_groups_and_primers.csv"))
}
