...
inflation_dispersion <- function(model) {
  ...
  zeroes_within_tolerance <- between(
    test2$ratio,
    1 - test2$tolerance,
    1 + test2$tolerance)
...
We verwijderen de stalen genomen in heide en beschouwen verder enkel de landgebruiksgradiënt akker - natuurgrasland.
...
metadata %>%
  ...
  ggplot(aes(x = landgebruik, y = value, colour = diepte)) +
  geom_violin() +
  geom_sina(alpha = 0.3) +
