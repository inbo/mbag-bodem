...
tidy_physeq %>%
  ...
  mutate(Landgebruik_MBAG = factor(
    Landgebruik_MBAG,
...
-   (optioneel) "rarefy" de samples tot een minimum aantal reads (instelbaar)
...
calc_ordinations <- function(
  ...
  if (!is.null(rarefy_min_reads)) {
    message(paste("samples rarefied to", rarefy_min_reads, "reads"))
    data <- data %>%
      tidytacos::add_total_count() %>%
      tidytacos::filter_samples(total_count >= rarefy_min_reads) %>%
      tidytacos::rarefy(n = rarefy_min_reads)
  }
