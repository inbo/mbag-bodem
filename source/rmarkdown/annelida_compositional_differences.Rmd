...
calc_ordinations <- function(
  ...
  data <- tacos_data %>%
    {if (!is.null(use_rank)) tidytacos::aggregate_taxa(., rank = use_rank)
      else .} %>%
...
plot_pca <- function(plotdata, colour = NULL) {
  ggpca <- plotdata %>%
    ggplot() +
    geom_point(
      aes(x = PC1, y = PC2, colour = {{colour}})
    ) +
    geom_vline(xintercept = 0, alpha = 0.3) +
    geom_hline(yintercept = 0, alpha = 0.3) +
    stat_ellipse(aes(
      x = PC1, y = PC2, colour = {{colour}}),
      level = 0.95) +
...
plot_tsne <- function(plotdata, colour = NULL) {
  ggtsne <- plotdata %>%
    ggplot() +
    geom_point(
      aes(x = V1, y = V2, colour = {{colour}})) +
    geom_vline(xintercept = 0, alpha = 0.3) +
    geom_hline(yintercept = 0, alpha = 0.3) +
    stat_ellipse(aes(
      x = V1, y = V2, colour = {{colour}}), level = 0.95) +
...
We fitten een multivariaat beta-binomiaal regressiemodel met diepte en landgebruik als fixed factoren en plot-id als random intercept [@mangiola2023].
