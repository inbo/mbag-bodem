...
ranks <- physeq_Olig01_Annelida_species %>%
  phyloseq::rank_names()
...
### Aggregatie op species niveau

```{r calc-ordinations-species}
ordination <- calc_ordinations(
  tacos_data = tidy_physeq,
  use_rank = "species",
  frequency_threshold = 0,
...
```{r calc-ordinations-genus}
ordination <- calc_ordinations(
  tacos_data = tidy_physeq,
  use_rank = "genus",
  frequency_threshold = 0,
  rarefy_even_reads = 30780
)
```
...
my_new_data <- sccomp_data_annelida %>%
...
s_annelida_predictions <- sccomp::sccomp_predict(
  s_annelida,
  formula_composition = ~
    Landgebruik_MBAG +
      Diepte +
...
s_annelida_predictions <- s_annelida_predictions %>%
...
Hier dus eens getest met de 293 meest prevalente OTUs

```{r sccomp-annelida-all-species}
fs::dir_create(here("source", "brms_models"))
if (!file.exists(here("source", "brms_models", "s_annelida_293_species.rds"))) {
  s_annelida_293_species <- sccomp_estimate(
    .data = sccomp_data_annelida,
    formula_composition = ~
      Landgebruik_MBAG
      + Diepte
  ...
  saveRDS(
    s_annelida_293_species,
    here("source", "brms_models", "s_annelida_293_species.rds")
  )
}

s_annelida_293_species <- readRDS(here("source", "brms_models", "s_annelida_293_species.rds"))
```

### Model evaluatie

```{r sccomp-test-species}
sctest <- sccomp::sccomp_test(s_annelida_293_species)
plots <- plot(sctest)
```
...
sortering <- s_annelida_293_species %>%
  filter(parameter == "Landgebruik_MBAGNatuurgrasland") %>%
  select(taxon_id, c_lower) %>%
  left_join(
    sccomp_data_annelida %>%
...
filtered_species_data <- s_annelida_predictions %>%
