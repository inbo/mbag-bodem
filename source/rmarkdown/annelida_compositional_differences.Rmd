...
Hier dus eens getest met alle OTUs (407)

```{r sccomp-annelida-all-species}
fs::dir_create(here("source", "brms_models"))
if (!file.exists(here("source", "brms_models", "s_annelida_407_species.rds"))) {
  s_annelida_407_species <- sccomp_estimate(
    .data = sccomp_data_annelida,
    formula_composition = ~
      Landgebruik_MBAG
    + Diepte
  ...
  saveRDS(s_annelida_407_species,
          here("source", "brms_models", "s_annelida_407_species.rds"))
}

s_annelida_407_species <- readRDS(here("source", "brms_models", "s_annelida_407_species.rds"))
```

### Model evaluatie

```{r sccomp-test-species}
sctest <- sccomp::sccomp_test(s_annelida_407_species)
plots <- plot(sctest)
```
...
sortering <- s_annelida_407_species %>%
  filter(parameter == "Landgebruik_MBAGNatuurgrasland") %>%
  select(taxon_id, c_lower) %>%
  left_join(sccomp_data_annelida %>%
