...
```{r sccomp-annelida-species}
fs::dir_create(here("source", "brms_models"))
if (!file.exists(here("source", "brms_models", "s_annelida_species.rds"))) {
  s_annelida_species <- sccomp_estimate(
    .data = sccomp_data_annelida,
    formula_composition = ~
      Landgebruik_MBAG
    + Diepte
  ...
  saveRDS(s_annelida_species,
          here("source", "brms_models", "s_annelida_species.rds"))
}

s_annelida_species <- readRDS(here("source", "brms_models", "s_annelida_species.rds"))
```

### Model evaluatie

```{r sccomp-test-species}
sctest <- sccomp::sccomp_test(s_annelida_species)
plots <- plot(sctest)
```
