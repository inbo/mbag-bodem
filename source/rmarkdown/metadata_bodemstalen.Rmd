...
Hoe belangrijk zijn de 0-10 cm versus 10-30 cm staalnamemonsters voor eDNA?



In `nematodenstalen` zitten alle plots waarvan ILVO nematodenstalen heeft binnengekregen en ge-extraheerd.
...
Een deel van de plots zijn momenteel nog in analyse, maar binnenkort hebben we wel alle fysico-chemische Cmon data (textuur, pH, C, N , bulk densiteit) en meta-data (foto's proefvlakken, vegetatiebeschrijving, condities bij staalname,...).
...
```{r landbouwstreken}
landbouwstreken <- read_sf(file.path("data", "landbouwstreken", "Lbstrbel.shp")) %>%
  janitor::clean_names() %>%
  st_transform(crs = 31370)
...
Om voor individuele strata-combinaties uitspraken te doen, hebben we dan echter vaak te weinig data.
...
```{r}
source("src/R/geocomputations.R")
...
Voorlopig enkel Leemstreek en Zandleemstreek (+ deel van de Polders).
