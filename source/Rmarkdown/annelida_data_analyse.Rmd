...
The raw number of taxa is a problematic measure because it depends on the total number of reads:
...
We therefore include the log of the total number of reads as a variable in the model.
For this particular analysis, we excluded sampling units for which the total number of reads was smaller than or equal to 100.
...
m_plant_micro <- brm(
  formula = form_plant_micro,
  data = env_microbiome_plant %>% filter(lib_size > 100),
  ...
  init = 0,
  iter = 1000,
  file = here("src", "brms_models", "m_plant_micro"),
